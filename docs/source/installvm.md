...
We recommend the Cloud Native Distribution for great performance and user experience. However, for development and testing we also support its VM distribution.
...
| Enter Openbanking static kid : GkwIzWy88xWSlcWnLiEc8ip9s2M| The fallback key when key is not passed in requests (as required by Brazil Openbanking)|
